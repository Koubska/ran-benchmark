...
public:
  ...
  void build_assignment(std::shared_ptr<Assignment> ass, std::vector<std::string> var_name,
                              std::vector<std::string> values) {

    for (size_t i = 0; i < var_name.size(); ++i) {
      ass->insert(std::make_pair(fresh_variable(var_name[i]), build_RAN(values[i]))) ;
    }
  }
